...
class LB130(object):
    ...
    def __init__(self, ip_address):
        ...
        if valid_ip:
            ...
            try:
                ...
                col4 = 'dft_on_state'
                try:
                    # if bulb is off, dft on state check
                    ...
                    self.__saturation = int(
                        data[col1][col2][col3]['saturation'])
                    self.__brightness = int(
                        data[col1][col2][col3]['brightness'])
                    self.__color_temp = int(
                        data[col1][col2][col3]['color_temp'])
                    ...
                    self.__saturation = int(
                        data[col1][col2][col3][col4]['saturation'])
                    self.__brightness = int(
                        data[col1][col2][col3][col4]['brightness'])
                    self.__color_temp = int(
                        data[col1][col2][col3][col4]['color_temp'])
                    self.device_id = str(data[col1][col2]['deviceId'])
    ...
    def on(self):
        ...
        "{\"smartlife.iot.smartbulb.lightingservice\":{\"\
                      ...
                      ,"dft_on_state":{' \
              '"mode":"normal","hue":195,"saturation":71,"color_temp":0,' \
              '"brightness":18}
        '''
        __bulb_on_off = 1
        msg = '{"smartlife.iot.smartbulb.lightingservice":{' \
    ...
    def __encrypt(value, key):
        ...
        if sys.version_info >= (3, 0):
            return bytearray(
                "".join(valuelist).encode("latin_1"))  # python 3 fix
        else:
